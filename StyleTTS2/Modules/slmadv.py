...
def _a2d(t):
    """[B,1,T] or [B,T] → [B,T] for WavLM input_values."""
...
class SLMAdversarialLoss(torch.nn.Module):
    ...
    def forward(
        ...
        if (iters + 1) % self.skip_update == 0:
            ...
            if use_rec:  # use reconstructed (shorter lengths), do length invariant regularization
                if wav.size(-1) > y_pred.size(-1):
                    ...
                    out_crop = self.wl.discriminator_forward(_a2d(real_GP.detach()))
                    out_org = self.wl.discriminator_forward(_a2d(wav.detach()))
                    loss_reg = F.l1_loss(out_crop, out_org[..., : out_crop.size(-1)])

                    if np.random.randint(0, 2) == 0:
                        d_loss = self.wl.discriminator(
                            _a2d(real_GP.detach()), _a2d(y_pred.detach())
                        ).mean()
                    else:
                        d_loss = self.wl.discriminator(
                            _a2d(wav.detach()), _a2d(y_pred.detach())
                        ).mean()
                else:
                    real_GP = y_pred[:, :, :crop_size]
                    out_crop = self.wl.discriminator_forward(_a2d(real_GP.detach()))
                    out_org = self.wl.discriminator_forward(_a2d(y_pred.detach()))
                    loss_reg = F.l1_loss(out_crop, out_org[..., : out_crop.size(-1)])

                    if np.random.randint(0, 2) == 0:
                        d_loss = self.wl.discriminator(
                            _a2d(wav.detach()), _a2d(real_GP.detach())
                        ).mean()
                    else:
                        d_loss = self.wl.discriminator(
                            _a2d(wav.detach()), _a2d(y_pred.detach())
                ...
                out_gt = self.wl.discriminator_forward(_a2d(y_rec_gt.detach()))
                out_rec = self.wl.discriminator_forward(_a2d(y_rec_gt_pred.detach()))
        ...
        # generator loss
        gen_loss = self.wl.generator(_a2d(y_pred))
